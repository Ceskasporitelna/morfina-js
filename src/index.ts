...
async function init() {
  try {
    ...
    const added = client.add(10, 2);
    const enc = await client.decryptField(added, null);
    console.log('enc', enc === '12');

    const result = await client.morph(payload);
    const encrypted = client.computer.encrypt(payload.dataArray.transactions[0].amount.value);
